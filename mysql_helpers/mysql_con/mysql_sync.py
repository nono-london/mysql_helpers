...
class MySQLConnectorNative:
    ...
    def __init__(self,
        ...
        self.db_port: Union[int, str] = environ['MYSQL_DB_PORT'] if db_port is None else str(db_port)
        self.db_user: str = environ['MYSQL_DB_USER'] if db_user is None else db_user
        self.db_password: str = environ['MYSQL_DB_PASS'] if db_password is None else db_password
    ...
    def open_connection(self) -> Union[None, MySQLConnection]:
        ...
        try:
            self.mysql_connection = MySQLConnection(host=self.db_host,
                                                    port=self.db_port,
                                                    user=self.db_user,
    ...
    def fetch_all_as_df(self, sql_query: str,
    ...
    def fetch_all_as_dicts(self, sql_query: str,
                           sql_variables: Optional[Tuple] = None,
                           close_connection: Optional[bool] = True) -> Union[List[Tuple], None]:
        """
            :param sql_query: the MySQL query
            :param sql_variables: parameters ordered with %s usage in the sql_query
            :param close_connection: close connection after the method ends
    ...
    def execute_one_query(self, sql_query: str,
                          sql_variables: Optional[Tuple] = None,
                          close_connection: Optional[bool] = True) -> int:
        """ method that handles execute queries: delete update insert
            :param sql_query: the MySQL query
            :param sql_variables: parameters ordered with %s usage in the sql_query
            :param close_connection: close connection after the method ends
            :return: returns the nube rof rows affected, -1 if error
        """
        ...
        try:
            ...
            # TODO: use the appropriate Excetion code for undefined variables
                ...
                print(f'Error while executing query : {ex}')
                print(f'Error while executing query : {ex.__class__.__name__}')
...
if __name__ == '__main__':
    ...
    print(my_getter.fetch_all_as_df(sql_query="SELECT * FROM tbl_proxy_url ORDER BY error_count DESC LIMIT 10 "))
